  ...
  return lastThree
    .map((part) => {
      ...
      clean = clean
        .split(/[-_]/g)
        .map((str, i) =>
          i > 0 ? str.charAt(0).toUpperCase() + str.slice(1) : str,
        )
        .join("");
  ...
  while (!_confirmed) {
    ...
    choices.push(
      new inquirer.Separator(
        chalk.dim(
          `─── 位置：${renderBreadcrumb(stack)}  已选 ${chalk.cyan(selectedCount)} 个 ───`,
        ),
      ),
    );
    ...
    if (selectedCount > 0) {
      choices.push({
        name: chalk.bgGreen.black(
          ` ✓ 确认生成 ${selectedCount} 个接口 → 直接回车 `,
        ),
    ...
    if (folders.length > 0) {
      ...
      for (const f of folders) {
        const folderSelected = collectAllApis(f.children).filter((a) =>
          selected.has(a.id),
        ).length;
        const badge =
          folderSelected > 0 ? chalk.cyan(` [已选${folderSelected}]`) : "";
        choices.push({
    ...
    if (apis.length > 0) {
      choices.push(
        new inquirer.Separator(chalk.dim("─── 接口（空格多选）───")),
      );
      // 当前层"全选/取消全选"快捷键
      const allCurrentSelected =
        apis.length > 0 && apis.every((a) => selected.has(a.id));
      choices.push({
...
export async function runInteractive(
  ...
  try {
    ...
    if (config.useProjectName) {
      ...
      const projectInfo = (userProjects as any[]).find(
        (p: any) => p.id === projectId,
      );
      const projectName = projectInfo?.name || "";
      if (projectName) {
        projectNameSegment = convertPathToPascalCase(
          cnToPinyin(projectName),
        ).trim();
      }
    }
  ...
  for (const [dir, items] of groupedByDir) {
    console.log(
      `  ${chalk.dim("→")} ${chalk.yellow(path.relative(cwd, dir))}  ${chalk.dim(`(${items.length} 个)`)}`,
    );
    for (const item of items) {
      console.log(
        `      ${colorMethod(item.method)} ${chalk.white(item.path)}`,
      );
    }
  }
  ...
  for (const [targetDir, items] of groupedByDir) {
    ...
    const genSpinner = ora(
      `生成 ${path.relative(cwd, targetDir)} (${ids.length} 个接口)...`,
    ).start();
    try {
      ...
      genSpinner.succeed(
        `已完成: ${chalk.cyan(path.relative(cwd, targetDir))}`,
      );
    } catch (err: any) {
