...
function flattenTreeWithPath(
  ...
  for (const node of tree) {
    if (node.type === "apiDetail" && node.api) {
      ...
      result.push(
        ...flattenTreeWithPath(node.children, [...groupPath, folderName]),
      );
    }
  }
  return result;
...
export async function runGenerate(
  ...
  try {
    ...
    if (config.useProjectName) {
      ...
      if (projectName) {
        rootGroupPath.push(
          convertPathToPascalCase(cnToPinyin(projectName)).trim(),
        );
      }
    }

    // ── 确定要生成的接口 IDs ──
    let targetApiItems: Array<{
      id: number;
      name: string;
      path: string;
      method: string;
    ...
    if (options.all) {
      ...
      console.log(
        chalk.red("请指定要生成的接口 ID/关键词，或使用 --all 生成全部"),
      );
      process.exit(1);
    }
