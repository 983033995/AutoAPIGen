...
import { firstToLocaleUpperCase, cnToPinyin } from '@auto-api-gen/core'
import type {
  ConfigFromModel,
  ApiDetailListData,
  ApiDataSchemasItem,
...
export function convertToTemplateString(
  ...
  return p.replace(/{(\w+)}/g, (_, varName) => {
    ...
    return p.includes(`\${${templateVar}}`) ? `{${varName}}` : `\${${templateVar}}`
  })
}
...
export function buildParameters(parameters: ApiDetailParametersQuery): string {
  ...
  const typeMap: Record<string, () => string> = {
    ...
    date: () => 'Date',
    string: () => {
      if (parameters?.enum) return parameters.enum.map((item: any) => `'${item}'`).join(' | ')
      return 'string'
    },
    integer: () => 'number',
    int64: () => 'number',
...
export function buildParametersSchema(
  ...
  if (!configObj) {
    ...
    if (bodyParameters.length === 0) return `export type ${interfaceName} = object;\n`
...
export function transformSchema(jsonSchema: Record<string, any>, interfaceName: string): string {
  ...
  const output = (obj: Record<string, any>, faceName: string): string => {
    ...
    if (type === 'object') {
      ...
      for (const key of keys) {
        ...
        const typeStr = buildPropertyType(property, containsCh ? cnToPinyin(key) : key, faceName)
  ...
  function buildChildrenOutput(childrenObj: Record<string, any>, childrenFaceName: string): string {
    ...
    const getRefObj = (ref: string): Record<string, any> => {
      ...
      return apiDataSchemas.find((item) => item.id === +refId)?.jsonSchema || {}
    }
    ...
    const schemaForInterface = type === 'array' ? noRef.items || childrenObj.items : noRef
    const isEmptyObject =
      !schemaForInterface?.properties || Object.keys(schemaForInterface.properties).length === 0
    if (isEmptyObject && type !== 'string') {
      return type === 'array' ? '{ [key: string]: any }[]' : '{ [key: string]: any }'
    }

    const descSource =
      schemaForInterface?.title || schemaForInterface?.description || noRef.title || noRef.description
...
function extractReturnData(responses: ApiDetailListData['responses'] | []): any {
  const returnDataKey: string[] = (_config.axiosReturnKey || '').split(',').filter((s) => s)
  const defaultResponse = (responses as any[])?.find((res) => +res.code === 200)
  ...
  if (returnDataKey.length) {
    const dataKeys = [...new Set([...(finalJsonSchema['x-apifox-orders'] || []), ...Object.keys(finalJsonSchema.properties || {})])].sort()
...
function resolveSchemaRef(
  ...
  if (jsonSchema?.$ref) {
    ...
    const schema = apiDataSchemas.find((item) => item.id === +refId)?.jsonSchema || {}
    if ((schema as any)?.$ref) return resolveSchemaRef(schema, apiDataSchemas, depth + 1, maxDepth)
    return schema
  }
  if (jsonSchema?.properties) {
    const resolvedSchema = { ...jsonSchema, properties: {} as Record<string, any> }
...
export async function formatCode(code: string, prettierSetting: Record<string, any>): Promise<string> {
  try {
...
function getPrettierSetting(): Record<string, any> {
  const defaultSetting = { semi: false, singleQuote: true, parser: 'typescript' }
  try {
    return { ...defaultSetting, ...JSON.parse(_config.prettierSetting || '{}') }
  } catch {
...
function buildMethodTemplate(
  ...
  try {
    ...
    const haveReqBody = Boolean(
      (apiDetailItem?.requestBody?.parameters || []).length || apiDetailItem?.requestBody?.jsonSchema
    )
    ...
    const exportInterfaceQuery = buildInterfaceQuery(apiFunctionName, apiDetailItem, queryParams)
    const exportInterfaceBody = buildInterfaceBody(apiFunctionName, apiDetailItem, haveReqBody)
    const exportInterfacePathQuery = buildInterfacePathQuery(apiFunctionName, apiDetailItem, pathParams)
    const exportInterfaceRes = buildInterfaceResponse(apiFunctionName, apiDetailItem, responses)
    ...
    const apiFunctionSignature = buildApiFunctionSignature(apiFunctionName, pathParams, queryParams, haveReqBody, apiMethod)
    const apiFunctionBody = buildApiFunctionBody(apiMethod, axiosAlias, apiPath, apiDetailParams, haveReqBody, queryParams, apiDetailItem)
    ...
    if (apiModel === 'custom') {
      ...
      const customFun = customFunctionReturn(options, description, defaultFunction, apiFunctionName)
    ...
    console.error(`构建方法模板失败 ${apiDetailItem.name} ${apiDetailItem.path}: ${error.message}`)
...
function getAllInterfaceNames(
  items: Array<{ interfaceQueryName: string; interfacePathQueryName: string; interfaceBodyQueryName: string; interfaceResName: string }>,
  allApiFunctionStr: string
): string {
  return Array.from(
    new Set(
      items.flatMap((cur) =>
        [cur.interfaceQueryName, cur.interfacePathQueryName, cur.interfaceBodyQueryName, cur.interfaceResName].filter(
          (item) => !!item && allApiFunctionStr.includes(item)
        )
      )
    )
  )
...
function buildHead(allInterfaceName: string, axiosQuote: string, isNeedQs: boolean): string {
...
export async function generateApiFiles(
  ...
  const items = apiIds.map((id) => {
    const detail: Partial<ApiDetailListData> = _detailList.find((d) => d.id === id) || {}
    const apiFunctionName = `${detail.method}${convertPathToPascalCase(detail.path || '')}`.trim()
    const useApiFunctionName = `use${firstToLocaleUpperCase(apiFunctionName)}`
    const { fun, interFace } = buildMethodTemplate(apiFunctionName, apiModel, detail, axiosQuote, projectId)
    return {
      id,
      apiFunctionName,
      useApiFunctionName,
      ...
      interfaceQueryName: (detail?.parameters?.query || []).length ? `${apiFunctionName}Query` : '',
      interfacePathQueryName: (detail?.parameters?.path || []).length > 1 ? `${apiFunctionName}PathQuery` : '',
      interfaceBodyQueryName:
        (detail?.requestBody?.parameters || []).length || detail?.requestBody?.jsonSchema
