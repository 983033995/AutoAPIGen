...
6. **AI 工具集成**：通过 `aag` CLI + Skill 注入，让 Windsurf、Cursor、Claude Code 等 AI 助手自动理解项目接口结构。

---

## AI 工具支持

AutoAPIGen 提供了专为 AI 编程助手设计的集成方案：

- **`aag` CLI**：配套命令行工具，AI 可直接调用查询接口信息、生成代码
- **Skill 注入**：一键将接口知识注入到 Windsurf、Cursor、Claude Code、GitHub Copilot 的规则文件

配置完成后，你可以直接告诉 AI：「帮我调用店铺列表接口」，AI 会自动查询接口结构并生成带完整类型的代码。

点击插件中的 **「启用 AI 支持」** 按钮查看完整安装教程 → [AI 工具支持文档](https://doc.du-ai.cn/guide/ai-support)
...
6. **AI 原生**：通过 CLI 和 Skill，让 AI 助手成为接口开发的真正协作者。
